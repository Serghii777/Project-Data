...
async def generate_parking_report(vehicle_id: uuid.UUID, db: AsyncSession):
    ...
    if not parking_records:
        raise HTTPException(status_code=404, detail="No parking records found for this vehicle.")
    ...
    with open(filename, 'w', newline='', encoding='utf-8') as csvfile:
        ...
        for record in parking_records:
            duration = (record.exit_time - record.entry_time).total_seconds() / 60  # Тривалість у хвилинах
            csvwriter.writerow([record.entry_time, record.exit_time, duration, record.cost])
